...
const DisputeWindow = ({ closeModel, updateDispute, item,consumerOrderID,socket,refetchOrder,setSelectedOrder}) => {
 ...
 const raiseDispute = async() => {
  ...
  try {
    ...
    if(response.ok){
      socket.emit('newDisputeRaised',{room:item.storeID,item:item.name,disputeReason:disputeReason.reason, storeName:item.storeName,webstoreUserID:item.webstoreUserID, productImage:item.productImage[0], date: new Date()})
      /* refetch item */
      refetchOrder()
