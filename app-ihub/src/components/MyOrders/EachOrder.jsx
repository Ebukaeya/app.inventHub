...
const itemOnDispute = () => {
  ...
  const resolveDisputeSimFun = async() => {
    ...
    try {
      ...
      if( response.ok){
        socket.emit('disputeResolved',{room:item.storeID,item:item.name, webstoreUserID:item.webstoreUserID, productImage:item.productImage[0], storeName:item.storeName,date: new Date()})
